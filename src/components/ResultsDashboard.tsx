...
export const ResultsDashboard: React.FC = () => {
  const { scores, maxScores, gender, resetQuiz, activeQuestions } = useStore();
  ...
  allDims.forEach(dim => {
      const s = scores[dim as keyof typeof scores] || 0;
      const m = maxScores[dim as keyof typeof maxScores] || 1; // avoid division by zero
      normalizedScores[dim] = Math.min(100, Math.round((s / m) * 100));
  });
  ...
  const chartData = allDims.map((dim) => {
      const dimName = dim as keyof typeof scores;
      const dataPoint: any = {
