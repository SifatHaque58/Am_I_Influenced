  ...
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/api/analyze' && request.method === 'POST') {
      ...
      try {
        ...
        const formattingRule = `- Use clean, standard markdown formatting.\n- Do NOT use unnecessary special characters, extra spacing, underscores, emojis, or conversational filler (like "Here is your analysis:" or "Hope this helps").\n- Strict adherence to the requested structure is required.`;

        const categories = Array.isArray(body.categories) && body.categories.length > 0 
          ? body.categories.join(', ') 
          : 'general daily life';
        const categoriesRule = `- The user was specifically quizzed on their behavior in these areas: ${categories}. Tailor your examples and insights to these areas when possible.`;
